    ...
    <div class="py-10">
        <div class="mx-auto max-w-3xl space-y-6 sm:px-6 lg:px-8">
            <div class="flex flex-wrap items-center gap-x-4 gap-y-2 px-4 text-sm sm:px-0">
                <a href="{{ route('agents.interview-preparations.index', $agent) }}" class="font-medium text-indigo-600 hover:text-indigo-800 hover:underline">&larr; Registos</a>
            ...
            <div class="overflow-hidden bg-white shadow-sm ring-1 ring-slate-200 sm:rounded-xl">
                <div class="border-b border-slate-100 bg-slate-50/80 px-6 py-5">
                    <h1 class="text-lg font-semibold text-slate-900">Registar entrevista</h1>
                ...
                <div class="px-6 py-6">
                    @if ($allProcessesClosedForNewEntry ?? false)
                        <p class="text-sm text-amber-900">
                            Todos os processos na biblioteca estão marcados como <strong>«Não prosseguiu»</strong> ao nível da candidatura global.
                            Não é possível registar novas rondas enquanto assim estiverem.
                        ...
                        <form method="post" action="{{ route('agents.interview-preparations.store', $agent) }}" class="space-y-5">
                            ...
                            <div>
                                ...
                                <select id="jd_document_id" name="jd_document_id" required
                                    ...
                                    <option value="" disabled @selected($preselectJd === null)>— Seleccionar —</option>
                            ...
                            <div class="rounded-lg bg-slate-50 px-4 py-3 text-sm ring-1 ring-inset ring-slate-100">
                                <p id="next-round-preview" class="font-medium text-slate-800">Seleccione um processo para ver a próxima ronda.</p>
                                <p class="mt-1 text-xs text-slate-500">A sequência corresponde à ronda (1, 2, 3…). É sempre a próxima livre neste processo e é atribuída ao guardar — não pode ser alterada manualmente aqui.</p>
                            </div>
                            <div>
                            ...
                            <div>
                                <label for="status" class="block text-sm font-medium text-slate-700">Estado do processo (apos esta ronda)</label>
                            ...
                            <div>
                                ...
                                <p class="mt-1 text-xs text-slate-500">O que podes tirar das sugestões e do diálogo com o agente no chat antes ou depois da ronda.</p>
                            ...
                            <div class="flex flex-wrap gap-2">
                                <button type="submit" class="inline-flex rounded-xl bg-violet-600 px-5 py-2.5 text-sm font-semibold text-white shadow-sm hover:bg-violet-700">Guardar</button>
                        ...
                        <script>
                            document.addEventListener('DOMContentLoaded', function () {
                                ...
                                function refresh() {
                                    var id = select.value;
                                    if (!id) {
                                        hint.textContent = 'Seleccione um processo para ver a próxima ronda.';
                                        return;
                                    }
                                    var n = map[id];
