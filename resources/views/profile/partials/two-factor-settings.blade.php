<section class="rounded-lg border border-gray-100 p-6">
    <header>
        ...
        <p class="mt-1 text-sm text-gray-600">
            Recomendado para maior segurança da conta após registar método de pagamento.
        </p>
    </header>

    @if ($user->two_factor_confirmed_at)
        <p class="mt-4 rounded-md bg-emerald-50 px-4 py-2 text-sm font-medium text-emerald-950">
            Dois passos está <strong>activo</strong>.
        </p>
        <form method="POST" action="{{ route('profile.two-factor.destroy') }}" class="mt-6 space-y-4">
            ...
            <div class="grid gap-4 sm:max-w-sm">
                ...
                <div>
                    <x-danger-button>Desactivar dois passos</x-danger-button>
                </div>
            </div>
        </form>
        ...
        @endif
    @else
        <p class="mt-4 text-sm text-gray-700">Ainda não activou o segundo factor.</p>
        <div class="mt-4">
