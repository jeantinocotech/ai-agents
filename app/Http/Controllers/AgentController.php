...
class AgentController extends Controller
{
    ...
    public function chat(Request $request, $id)
    {
        ...
        return view('agents.chat', compact(
            'agent',
            'steps',
            'session',
            'tokenBalance',
    ...
    public function getCurrentStep(Request $request, $agentId)
    {
        ...
        if (! $session) {
            ...
            if (! $firstStep) {
                ...
                $cvJdPair = $this->findCvJdStepPair($agentId);
                if ($cvJdPair !== null) {
                    $defaults = AgentDocumentDefault::query()
                        ->where('user_id', $user->id)
                        ->where('agent_id', $agentId)
                        ->with('defaultCvDocument')
                        ->first();
                    $savedCvBody = $defaults?->defaultCvDocument
                        ? trim((string) $defaults->defaultCvDocument->body)
                        : '';
                } else {
                    $savedCvBody = '';
                }

                if ($cvJdPair !== null && $savedCvBody !== '') {
                    $jdStep = $cvJdPair['jd'];
                    $session = ChatSession::create([
                        'user_id' => $user->id,
                        'agent_id' => $agentId,
                        'ai_model' => $agent->model_type,
                        'current_step' => $jdStep->step_order,
                        'should_persist' => true,
                        'is_active' => true,
                    ]);
                    ...
                    ChatMessage::query()->create([
                        ...
                        'role' => 'user',
                        'content' => "Conteúdo do CV reutilizado (guardado no seu perfil para este agente):\n\n".$savedCvBody,
                    ]);

                    Log::info('Nova sessão com CV guardado — início no passo JD', [
                        'session_id' => $session->id,
                        'jd_step' => $jdStep->step_order,
                    ]);
                    ...
                    return response()->json([
                        ...
                        'current_step' => $jdStep->step_order,
                        'required_input' => $jdStep->required_input,
                        'upload_file' => (bool) $jdStep->upload_file,
                        'reused_saved_cv' => true,
                    ]);
                }
        ...
        try {
            ...
            if (! $this->tokenWallet->userHasMinimumBalance($user)) {
                return response()->json([
                    ...
                    'message' => 'Seus tokens acabaram ou estão abaixo do mínimo. Compre mais ou aguarde a renovação.',
            ...
            $billing = $this->apiUsageBilling->computeDebit($usage, $provider, $aiModel);
            $this->tokenWallet->debitUsage(
                $user->fresh(),
                $billing['debit'],
                [
                    'provider' => $provider,
                    ...
                    'api_prompt_tokens' => $billing['api_prompt_tokens'],
                    'api_completion_tokens' => $billing['api_completion_tokens'],
                ],
                ChatMessage::class,
                $assistantMessage->id
            );

            $tokenBalance = (int) $user->fresh()->token_balance;
    ...
    public function sendFile(Request $request)
    {
        try {
            ...
            if ($request->hasFile($inputType)) {
                ...
                if ($currentStep && (int) $currentStep->current_step > 0) {
                    $session->current_step = (int) $currentStep->current_step;
                    $session->save();
                } else {
                    return response()->json([
                        'session_id' => $request->session_id,
                ...
                $nextStepModel = AgentStep::query()
                    ->where('agent_id', $request->agent_id)
                    ->where('step_order', $session->current_step)
                    ->first();

                $offerSaveCv = strtolower(trim($inputType)) === 'cv'
                    && $this->findCvJdStepPair($request->agent_id) !== null;

                return response()->json([
                    'session_id' => $request->session_id,
                    'current_step' => (int) $currentStep->current_step,
                    'next_required_input' => $currentStep->required_input,
                    'next_upload_file' => (bool) ($nextStepModel->upload_file ?? false),
                    'offer_save_cv' => $offerSaveCv,
                ]);
    ...
    /**
     * @return array{cv: \App\Models\AgentStep, jd: \App\Models\AgentStep}|null
     */
    private function findCvJdStepPair(int $agentId): ?array
    {
        $steps = AgentStep::query()
            ->where('agent_id', $agentId)
            ->orderBy('step_order')
            ->take(2)
            ->get();
        ...
        $first = strtolower(trim((string) $steps[0]->required_input));
        $second = strtolower(trim((string) $steps[1]->required_input));

        if ($first === 'cv' && $second === 'jd') {
            return ['cv' => $steps[0], 'jd' => $steps[1]];
        }

        return null;
    }

    public function storeSavedCv(Request $request)
    {
        ...
        if ((int) $session->user_id !== (int) $user->id || (int) $session->agent_id !== (int) $validated['agent_id']) {
        ...
        try {
            ...
            return response()->json([
                'message' => collect($e->errors())->flatten()->first() ?? 'O texto do CV excede o limite permitido.',
                'errors' => $e->errors(),
            ], 422);
        }
        ...
        $doc = AgentDocument::query()->create([
            ...
            'title' => 'CV guardado — '.now()->format('d/m/Y H:i'),
            'body' => $body,
        ]);
    ...
    {
        $request->merge([
            'agent_id' => $request->query('agent_id', $request->input('agent_id')),
        ]);
    ...
    private function extractLatestCvBodyFromSession(int $chatSessionId): ?string
    {
        ...
        foreach ($messages as $msg) {
            ...
            if (preg_match('/Conteúdo do arquivo\s*"cv"\s*extraído automaticamente:\s*\R\s*\R(.*)/is', $content, $m)) {
                $extracted = trim($m[1]);
     ...
     */
    private function callOpenAI($agent, $messageHistory): ?array
    {
        ...
        try {
            ...
            $json = $response->json();
            $text = $json['choices'][0]['message']['content'] ?? 'Sem resposta do modelo.';
            $usage = $json['usage'] ?? [];

            return ['text' => $text, 'usage' => is_array($usage) ? $usage : []];
        } catch (\Exception $e) {
     ...
     * @return array{text: string, usage: array<string, mixed>}|null
     */
    private function callAnthropic($agent, $messageHistory): ?array
    {
        ...
        try {
            ...
            $json = $response->json();
            $text = $json['content'][0]['text'] ?? 'Sem resposta do Claude.';
            $usage = $json['usage'] ?? [];
     ...
     * @return array{text: string, usage: array<string, mixed>}|null
     */
    private function callOpenAIUsingAssistant($agent, $session, $userMessage): ?array
    {
        ...
        $runUsage = $runCheck['usage'] ?? [];
        if (! is_array($runUsage)) {
            $runUsage = [];
        }
